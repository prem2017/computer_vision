...
def generate_report(model, data_loader, image_names=None):
	...
	with torch.no_grad():
		for i, xy in enumerate(data_loader):
			...
			if label != -1: # if label is not none
	...
	for i, y_pred in enumerate(ypred_labels_all):
		for k, v in util.CLASS_TO_PIXEL_MAP.items():
			y_pred[y_pred == k] = v
		if image_names is not None:
			img_name = image_names[i]
		else:
			img_name = str(i)
		io.imsave(os.path.join(util.RESULT_DATAPATH, 'result_' + img_name + '.png'), y_pred)
...
if __name__ == '__main__':
	...
	test_dir = 'testing'
	img_list_fname = 'test_iminfo_nolabel.csv' # has_labels
	...
	transformer = torch_transformer.Compose([RescaleImage((util.HEIGHT_DEFAULT_VAL, util.WIDTH_DEFAULT_VAL)), # RandomCropImage((util.HEIGHT_DEFAULT_VAL, util.WIDTH_DEFAULT_VAL))
	                                         ToTensor(),
	                                         NormalizeImageData()
	])
