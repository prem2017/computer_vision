...
def train_network(dataloader, model, loss_function, optimizer, start_lr, end_lr, num_epochs=90):
	...
	# TODO: for finding optimal learning rate
...
def train_convnet(datatype_dir, img_list_fname):
	...
	transformer = torch_transformer.Compose([RescaleImage((2*util.HEIGHT_DEFAULT_VAL, 2*util.WIDTH_DEFAULT_VAL)), RandomCropImage((util.HEIGHT_DEFAULT_VAL, util.WIDTH_DEFAULT_VAL)), # RescaleImage((2 * util.HEIGHT_DEFAULT_VAL, 2 * util.WIDTH_DEFAULT_VAL)), convert the data between 0-1
	                                         ToTensor(),
	                                         NormalizeImageData()
	])
